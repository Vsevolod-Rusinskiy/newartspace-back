...
export class RequestFormService {
  ...
  private createCartItemHTML(painting: Painting): string {
    ...
    if (painting.priceType === 'Скидка' && painting.discount) {
      ...
      originalPriceHTML = `<span style="text-decoration: line-through; color: #999; margin-right: 8px;">${painting.price} ₽</span>`
    ...
    return `
    <div style="display: flex; margin-bottom: 20px; padding: 15px; border: 1px solid #eaeaea; border-radius: 5px; background-color: #fff;">
      ...
      <div style="flex: 1; display: flex; flex-direction: column;">
        <h2 style="font-size: 18px; font-weight: 600; color: #333; margin-top: 0; margin-bottom: 10px;">${painting.title}</h2>
        
        <div style="font-size: 14px; color: #666; margin-bottom: 10px;">
          <p style="margin: 5px 0;"><span style="color: #999;">Автор:</span> ${painting.artist?.artistName || 'Не указан'}</p>
          <p style="margin: 5px 0;"><span style="color: #999;">Размер:</span> ${dimensions}</p>
          <p style="margin: 5px 0;"><span style="color: #999;">Материалы:</span> ${materials}</p>
          <p style="margin: 5px 0;"><span style="color: #999;">Год:</span> ${painting.yearOfCreation || 'Не указан'}</p>
        </div>
      </div>
  ...
  private createEmailTemplate(
    ...
    <head>
      ...
      <style>
        ...
        .footer { text-align: center; margin-top: 30px; padding: 20px 0; border-top: 1px solid #eaeaea; font-size: 12px; color: #666; }
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1 style="color: #333; margin: 0;">Новое пространство</h1>
          <p style="color: #666;">Галерея молодых и малоизвестных художников</p>
        </div>
